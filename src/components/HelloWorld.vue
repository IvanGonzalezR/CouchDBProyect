...
<script lang="ts">
  ...
  export default Vue.extend({
    ...
    methods:{
      cargardatos(){
        axios.get("http://192.168.100.116:5984/tareas/_all_docs?include_docs=true").then((result) => {
      ...
      agregarTarea(){
        axios.post("http://192.168.100.116:5984/tareas", this.newtarea).then((result) => {
        this.cargardatos();
      ...
      eliminarTarea(id: string | number){
        ...
        axios.delete("http://192.168.100.116:5984/tareas/"+ this.listatareas[id].id + 
      ...
      modificarTarea(){
        if(this.tareaSelectedIndex>=0){
          ...
          let Lrev=this.listatareas[this.tareaSelectedIndex].doc._rev;
          axios.put("http://192.168.100.116:5984/tareas/"+ Ltarea.id, Ltarea.doc, {
            params:{"_rev": Lrev},
